...
int main() {
	...
	for (int row = 0; row < imageHeight; ++row) {
		...
		for (int col = 0; col < imageWidth; ++col) {
			Color color(static_cast<double>(row) / static_cast<double>(imageHeight - 1), 
				static_cast<double>(col) / static_cast<double>(imageWidth - 1), 0.25);
			...
			const auto ucg = static_cast<unsigned char>(color.r8bit());
			const auto ucb = static_cast<unsigned char>(color.r8bit());
			// store pixel data
